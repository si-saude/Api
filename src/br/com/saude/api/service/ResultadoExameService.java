...
implements GenericService<ResultadoExame, ResultadoExameFilter>{
	...
	public Response importFile(File arquivo) {
		try {
			String msn = ResultadoExameBo.getInstance().importFile(arquivo);
			if ( msn.length() > 0 )
				return Response.ok("As seguintes matriculas não foram salvas: " + msn).build();
			else return Response.ok("Salvo com sucesso.").build();
		} catch (Exception e) {
			e.printStackTrace();
			return Response.status(Response.Status.NOT_ACCEPTABLE).entity(e.getMessage()).build();
		}
