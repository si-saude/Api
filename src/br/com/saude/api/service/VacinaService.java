...
public class VacinaService {
	...
	@Path("/list")
	public Response getList(VacinaFilter filter) throws Exception {
		return Response.ok(VacinaBo.getInstance().getList(filter).getGenericPagedList()).build();
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Path("/selectList")
	public Response getSelectList(VacinaFilter filter) throws Exception {
		return Response.ok(VacinaBo.getInstance().getSelectList(filter)).build();
	}
	
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Response get(@QueryParam("id") int id) throws Exception{
		return Response.ok(VacinaBo.getInstance().getById(id)).build();
	}
	...
	public Response delete(int id) {
		try {
			VacinaBo.getInstance().delete(id);
