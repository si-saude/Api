	...
	public Aprho save(Aprho aprho) throws Exception {
		...
		if(aprho.getAprhoEmpregados() != null ) {
			...
			aprho.getAprhoEmpregados().forEach(e -> {
				if(e.isAtual()) {
					// 1 - Configurar um objeto do tipo AprhoEmpregadoFilter, passando o empregado atual,
					...
					// 2 - Adicionar os registros retornados na lista de AprhoEmpregados, SE O ID DA APRHO FOR DIFERENTE DA 
					// APRHO QUE ESTÁ VINDO DO FRONT
				}
			});
		}
...
//		
		...
		try {
			...
			stringReplacer = stringReplacer.replace("logoPetrobras", "data:image/png;base64," + Base64.getEncoder().encodeToString(logoArray));
		...
		List<Gerencia> gerenciaSetoriais = aprho.getAprhoEmpregados().stream()
			...
			.map(aE -> aE.getEmpregado().getGerencia())
		...
		String Aprovador =aprho.getAprovador().getEmpregado().getPessoa().getNome()+"- Mat."+aprho.getAprovador().getEmpregado().getMatricula();
				
		stringReplacer = stringReplacer
			.replace("gheNome", Objects.toString(aprho.getGhe().getNome()))
			.replace("revisao", Objects.toString(aprho.getRevisao()))
			.replace("empresa", Objects.toString(aprho.getEmpresa()))
			.replace("unidadeOrganizacional", unidadesString)
			.replace("gerencia", gerenciaString)
			.replace("gerenciaSetorial", gerenciaSetoriaisString)
			.replace("Cargo", cargosEmpregados)
			.replace("numeroTrabalhadores",	Objects.toString(aprho.getAprhoEmpregados().stream().count()))
			.replace("apravadorMatricula", Aprovador)
			.replace("elaboradores",elaboradores);
