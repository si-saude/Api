...
public class ConvocacaoBo
		extends GenericBo<Convocacao, ConvocacaoFilter, ConvocacaoDao, ConvocacaoBuilder, ConvocacaoExampleBuilder> {
	...
	public Convocacao getEmpregadoConvocacoesByGerencia(Convocacao convocacao) throws Exception {
		...
		if (gerenciaConvocacao.getId() > 0) {
			convocacao.setEmpregadoConvocacoes(ConvocacaoBuilder.newInstance(getDao().getByIdGerencia(convocacao))
		...
		List<Empregado> empregados = EmpregadoBo.getInstance().getListFunctionLoadGrupoMonitoramentosExames(filter).getList();
		...
		empregados.forEach(e -> {
			...
			if (convocacao.getEmpregadoConvocacoes().stream().filter(eC -> eC.getEmpregado().getId() == e.getId())
					.count() == 0) {
				...
				for (int i = 0; i < convocacao.getEmpregadoConvocacoes().size(); i++) {
					if (convocacao.getEmpregadoConvocacoes().get(i).getEmpregado().getId() == e.getId()) {
	...
	public Convocacao getById(Object id) throws Exception {
		...
		return ConvocacaoBuilder.newInstance(convocacao).loadEmpregadoConvocacoesAll().loadGerenciaConvocacoes()
	...
	private List<Exame> getExames(Empregado empregado, Convocacao convocacao) throws Exception {
		Profissiograma profissiograma = ProfissiogramaBo.getInstance().getById(convocacao.getProfissiograma().getId());
		...
		grupoMonitoramentos.forEach(g -> {
			if (g.getGrupoMonitoramentoExames() != null) {
				...
				g.getGrupoMonitoramentoExames().forEach(gE -> {
					...
					loop: for (Criterio criterio : gE.getCriterios()) {
						String valor = "";
